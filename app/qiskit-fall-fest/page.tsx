...
export default function QiskitFallFestPage() {
  const ctaHref = qiskitFallFest.registrationUrl ?? siteConfig.social.discord;
      ...
      <main className="relative flex-grow">
        ...
        <section className="relative w-full h-[60vh] min-h-[420px]">
          <Image
            ...
            alt="SQE quantum lab at Colorado School of Mines"
            fill
            priority
            className="object-cover"
          />
          <div className="absolute inset-0 bg-black/55" />
          <div className="absolute top-0 left-0 right-0 h-32 bg-gradient-to-b from-[#181818] to-transparent z-10" />
          <div className="absolute bottom-0 left-0 right-0 h-32 bg-gradient-to-t from-[#181818] to-transparent z-10" />
          <div className="absolute inset-0 flex flex-col items-center justify-center text-center px-6">
            <motion.p
              className="text-[#c5b783] text-sm sm:text-base font-semibold tracking-wide uppercase mb-4"
            ...
            <motion.h1
              className="font-display tracking-tight text-4xl sm:text-5xl md:text-6xl lg:text-7xl font-bold leading-tight"
        ...
        <section className="px-6 pb-16 md:pb-24">
          <div className="mx-auto max-w-6xl">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 border border-white/10 overflow-hidden">
              {qiskitFallFest.highlights.map((item, index) => {
                const Icon = highlightIcons[index] ?? Cpu;
                return (
                  <motion.div
                    key={item.title}
                    className="px-8 py-10 border-r border-b border-white/10 hover:bg-white/[0.03] transition-colors"
                    initial={{ opacity: 0, y: 20 }}
                    whileInView={{ opacity: 1, y: 0 }}
                    viewport={{ once: true }}
                    transition={{ delay: index * 0.08, duration: 0.5 }}
                  >
                    <Icon className="w-6 h-6 text-[#c5b783] mb-4" />
          ...
          <div className="mx-auto max-w-6xl">
            ...
            <div className="border border-white/10 overflow-hidden">
              {qiskitFallFest.schedule.map((item, index) => (
                <motion.div
                  key={item.day}
                  className="flex flex-col sm:flex-row gap-4 sm:gap-8 px-8 py-8 border-b border-white/10 last:border-b-0 hover:bg-white/[0.03] transition-colors"
                ...
                >
                  <div className="sm:w-44 shrink-0">
                    <p className="text-[#c5b783] text-sm font-semibold">
                      {item.day}
                    </p>
                    <p className="text-zinc-500 text-sm mt-1">{item.time}</p>
                  </div>
                  <div>
        ...
        <section className="px-6 pb-20">
          <motion.div
            className="mx-auto max-w-3xl text-center border border-white/10 px-8 py-14"
            initial={{ opacity: 0, y: 20 }}
            whileInView={{ opacity: 1, y: 0 }}
            viewport={{ once: true }}
          >
